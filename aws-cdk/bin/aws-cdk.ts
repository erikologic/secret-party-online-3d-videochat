...
const domain = getEnvVar(
    "DUCKDNS_DOMAIN",
    "You need to provide your DuckDNS (sub) domain"
);
const token = getEnvVar(
    "DUCKDNS_TOKEN",
    "You need to provide your DuckDNS token"
);
const email = getEnvVar(
    "EMAIL",
    "You need to provide an email for signing the Let's Encrypt HTTPS certificates"
);
...
const stackConfig: cdk.StackProps = {
    env: {
        account: getEnvVar(
            "AWS_ACCOUNT",
            "You need to provide an AWS_ACCOUNT number"
        ),
