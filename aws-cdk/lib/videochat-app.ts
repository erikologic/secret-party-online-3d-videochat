...
export class VideochatApp extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props: cdk.StackProps, appConfig: AppConfigProps) {
        ...
        securityGroup.addIngressRule(Peer.anyIpv4(), Port.tcp(3478), 'allow turn access');
        securityGroup.addIngressRule(Peer.anyIpv4(), Port.udp(3478), 'allow turn access');
        securityGroup.addIngressRule(Peer.anyIpv4(), Port.udpRange(49152, 65535), 'allow turn access');

        const userData = UserData.forLinux();
        userData.addCommands(`export DUCKDNS_DOMAIN=${appConfig.duckDns.domain}`);
        userData.addCommands(`export DUCKDNS_TOKEN=${appConfig.duckDns.token}`);
        userData.addCommands(`export URL=${appConfig.duckDns.domain}.duckdns.org`);
        userData.addCommands(`export EMAIL=${appConfig.email}`);
        const deployScript = readFileSync(path.resolve(process.cwd(), './script/deploy.sh'), { encoding: 'utf-8' })
        const execDeployScriptWithLogging = `sudo --preserve-env=DUCKDNS_DOMAIN,DUCKDNS_TOKEN,URL,EMAIL -u ec2-user bash -c '${deployScript}' 2>&1 |` +
